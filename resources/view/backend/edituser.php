...
if ($gambar != "") {
  ...
  if (in_array($ekstensi, $ekstensi_diperbolehkan) === true) {
    move_uploaded_file($file_tmp, '../boostrap/assets/img/user/' . $nama_gambar_baru); //memindah file gambar ke folder gambarke folder gambar

    // jalankan query UPDATE berdasarkan id_about yang produknya kita edit
    $query  = "UPDATE login SET nama_lengkap = '$nama_lengkap', username = '$username',password = '$password', id_role = '$id_role', gambar = '$nama_gambar_baru'";
  ...
  // jalankan query UPDATE berdasarkan id_about yang produknya kita edit
  $query  = "UPDATE login SET nama_lengkap = '$nama_lengkap', username = '$username', id_role = '$id_role',";
  $query .= "WHERE id_login = '$id_login'";
  $result = mysqli_query($koneksi, $query);
  // periska query apakah ada error
  if (!$result) {
