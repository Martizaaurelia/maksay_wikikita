...
$menu_image = $_FILES['image']['name'] ?? ''; // Assuming file upload
...
if ($menu_image) {
    ...
    if (isValidImage($extension)) {
        ...
        if (move_uploaded_file($file_tmp, '../../../bootstrap/assets/img/menu/' . $new_image_name)) {
            ...
            $query = "UPDATE menu SET menu_code=?, menu_name=?, menu_description=?, menu_image=? WHERE id_menu=?";
            $stmt = $koneksi->prepare($query);
            $stmt->bind_param("ssssi", $menu_code, $menu_name, $menu_description, $new_image_name, $id_menu);
        } else {
    ...
    // Prepare the UPDATE query without an image
    $query = "UPDATE menu SET menu_code=?, menu_name=?, menu_description=? WHERE id_menu=?";
    $stmt = $koneksi->prepare($query);
    $stmt->bind_param("sssi", $menu_code, $menu_name, $menu_description, $id_menu);
}
...
if ($success) {
    echo "<script>alert('Menu item updated successfully.'); window.location='../../menu.php';</script>";
} else {
